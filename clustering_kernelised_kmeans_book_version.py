...
import random as r
from data import observed_data_classification, observed_data_classification_two_features, observed_data_classification_two_features_nonlinear
...
X, true_y, X_means, X_covs = observed_data_classification_two_features_nonlinear(
    n,
    mean_data,
    cov_data,
)
...
def compute_mus(K, X, assignment_matrix):
    ...
    new_mu = (assignment_matrix *
              X_K).sum(axis=0) / assignment_matrix.sum(axis=0)
    return new_mu
